$(function() {
	$('#slider ul.slider-items').jcarousel(
			{
				...
				itemVisibleInCallback : {
					onAfterAnimation : function(c, o, i, s) {
						...
						jQuery('.slider-nav li:eq(' + (i - 1) + ')').addClass(
								'active');
					}
				}
			});
	...
	Cufon.replace(
			'.slider-items h2, .slider-items h3, .slider-items h4:not(.short)',
			{
	...
	if ($.browser.msie && $.browser.version == 6) {
		DD_belatedPNG
				.fix('#wrapper, h1#logo a, .shopping, #top-nav a, #navigation, .dd a, #slider, .button , .button span, .more, .product .price, .price .inner, #product-slider .info, #product-slider .jcarousel-prev, #product-slider .jcarousel-next, #footer, .boxes, .copy, .box-entry img, .social-box img');
	}
});
...
function mycarousel_initCallback(carousel) {
	...
	$('.slider-nav a').bind(
			'click',
			function() {
				carousel.scroll(jQuery.jcarousel.intval((jQuery(this).parent()
						.index()) + 1));
				return false;
			});
};